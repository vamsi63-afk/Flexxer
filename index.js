...
client.on('ready', async () => {
  ...
  const commands = [
    ...
    { name: 'mute', description: 'Mute a member', options: [{ name: 'user', description: 'Member', type: 6, required: true }] },
    { name: 'unmute', description: 'Unmute a member', options: [{ name: 'user', description: 'Member', type: 6, required: true }] },
    { name: 'deafen', description: 'Deafen a member', options: [{ name: 'user', description: 'Member', type: 6, required: true }] },
    { name: 'undeafen', description: 'Undeafen a member', options: [{ name: 'user', description: 'Member', type: 6, required: true }] },
    { name: 'vkick', description: 'Disconnect member from VC', options: [{ name: 'user', description: 'Member', type: 6, required: true }] },
...
riffy.on('trackStart', async (player, track) => {
  ...
  try {
    ...
    const msg = await channel.send({
      content: `**<a:playy:1477532288274272387> Now Playing**

> **[${track.info.title}](${track.info.uri})** - \`${artist}\`
> Duration: \`${duration}\`
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'mute') {
    ...
    if (!user.voice.channel) {
      return interaction.reply({ content: '❌ User is not in voice channel', ephemeral: true });
    }
    await user.voice.setMute(true);
    return interaction.reply(`🔇 ${user.user.tag} muted`);
  }
  ...
  if (commandName === 'unmute') {
    ...
    return interaction.reply(`🔊 ${user.user.tag} unmuted`);
  }
  ...
  if (commandName === 'deafen') {
    ...
    return interaction.reply(`🎧 ${user.user.tag} deafened`);
  }
  ...
  if (commandName === 'undeafen') {
    ...
    return interaction.reply(`🎵 ${user.user.tag} undeafened`);
  }

  if (commandName === 'vkick') {
    const user = options.getMember('user');
    if (!user.voice.channel) {
      return interaction.reply({ content: '❌ User is not in voice channel', ephemeral: true });
    }
    await user.voice.disconnect();
    return interaction.reply(`👢 ${user.user.tag} disconnected from VC`);
  }
    ...
    player = riffy.createConnection({
      ...
      if (resolve.loadType === 'playlist') {
        ...
        await interaction.editReply({
          content: `**Track Queued**\n> <:tick:1472560255396347999> **[${track.info.title}](${track.info.uri})** added to queue. Artist: ${artist}`
        });
      } else {
        return interaction.editReply({ content: `${config.emojis.error} No results found` });
      }
...
if (message.content.startsWith(`${prefix}prefix`)) {
    ...
    if (!newPrefix)
        return message.reply("❌ Provide new prefix.");
...
// VOICE MODERATION CODE HERE
          if (command === 'voice' || command === 'vc') {
    ...
    if (sub === 'mute') {
        ...
        if (!member?.voice?.channel)
            return message.reply("❌ User not in VC");

        await member.voice.setMute(true);
        return message.reply("✅ User muted");
    }
    ...
    if (sub === 'deafen') {
        ...
        return message.reply("✅ User deafened");
    }
    ...
    if (sub === 'undeafen') {
        ...
        return message.reply("✅ User undeafened");
    }
    ...
    if (sub === 'move') {
        ...
        if (!target)
            return message.reply("❌ Invalid VC ID");

        await member.voice.setChannel(target);

        return message.reply("✅ User moved");
    }
    ...
    if (sub === 'moveall') {
        ...
        if (!target)
            return message.reply("❌ Invalid target VC");

        let source = message.member.voice.channel;

        if (sourceId) {
            source = message.guild.channels.cache.get(sourceId);
        }

        if (!source)
            return message.reply("❌ Source VC not found");

        source.members.forEach(m => {
            m.voice.setChannel(target).catch(() => {});
        });

        return message.reply("✅ Everyone moved");
    }
                                 }
  if (command === 'support') {
            const container = new ContainerBuilder()
